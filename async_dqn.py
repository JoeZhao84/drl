...
flags.DEFINE_string('game', 'Breakout-v0', 'Nlame of the atari game to play. Full list here: https://gym.openai.com/envs#atari')
...
flags.DEFINE_integer('anneal_epsilon_timesteps', 10000, 'Number of timesteps to anneal epsilon.') #1000000
flags.DEFINE_string('summary_dir', 'logs/summaries', 'Directory for storing tensorboard summaries')
flags.DEFINE_string('checkpoint_dir', 'logs/checkpoints', 'Directory for storing model checkpoints')
...
flags.DEFINE_string('eval_dir', 'logs/', 'Directory to store gym evaluation')
...
def train(session, graph_ops, num_actions, saver):
    ...
    saver.restore(session, "logs/breakout.ckpt-1000800")

    summary_save_path = FLAGS.summary_dir + "/" + FLAGS.experiment + "/" + (datetime.datetime.now().strftime("%Y%m%d%H%M%S"))
...
def evaluation(session, graph_ops, saver):
    ...
    monitor_env.monitor.start(FLAGS.eval_dir+"/"+FLAGS.experiment+"/eval"+"/"+(datetime.datetime.now().strftime("%Y%m%d%H%M%S")))
...
def main(_):
  ...
  with g.as_default(), tf.Session() as session:
    ...
    saver = tf.train.Saver() #if it's using restore
